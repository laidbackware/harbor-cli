...
type ConfigurationsResponse struct {
	...
	// By default its empty so the email_username is picked
	...
	// When its set to true the system will access Email server via TLS by default.  If its set to false, it still will handle 'STARTTLS' from server side.
	...
	// Verify the HTTP auth providers certificate
	...
	// Verify the OIDC providers certificate
	...
	// Whether the Harbor instance supports self-registration.  If its set to false, admin need to add user to the instance.
...
type ConfigurationsResponseScanAllPolicyParameter struct {
	...
	// The offset in seconds of UTC 0 oclock, only valid when the policy type is 'daily'
